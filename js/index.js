...
(async function(){
  ...
  try {
    ...
    function renderList(){
      ...
      if(state.tab === "視聴者数"){
        const sorted = [...DB.cache.songs].sort((a,b)=>b.viewer_count - a.viewer_count).slice(0, limit);
        ...
        const sorted = [...DB.cache.songs].sort((a,b)=> (applyLikeCount(b.likes, b.id)) - (applyLikeCount(a.likes, a.id))).slice(0, limit);
        ...
        const likeByArtist = {};
        DB.cache.songs.forEach(s=>{
          likeByArtist[s.artist_id] = (likeByArtist[s.artist_id]||0) + applyLikeCount(s.likes, s.id);
        });
        const rows = DB.cache.artists.map(a=>({artist:a, tot: likeByArtist[a.id]||0}))
